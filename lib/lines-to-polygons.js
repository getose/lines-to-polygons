...
var linesToPolygons = function linesToPolygons(lines) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var currentLine = lines.splice(i--, 1)[0]; // create a new polygon array

    var polygon = []; // put the first line on the polygon array
    ...
    while (lines.length && (j < lines.length || linesLength != lines.length)) {
      ...
      if (currentend.x === nextLine.start.x && currentend.y === nextLine.start.y) {
        ...
        currentend = nextLine.end; // Suppression de la ligne dans le tableau
      ...
      if (polygon.length >= 3 && polygon[0].line.start.x == polygon[polygon.length - 1].line.end.x && polygon[0].line.start.y == polygon[polygon.length - 1].line.end.y) {
